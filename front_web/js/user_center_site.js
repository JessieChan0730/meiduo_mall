var vm = new Vue({
    ...
    mounted: function () {
        ...
        // 获取用户地址列表
        axios.get(this.host + '/auth' + '/addresses/', {
            headers: {
    ...
    methods: {
        ...
        save_address: function () {
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id) {
                ...
                if (this.editing_address_index === '') {
                    // 新增地址
                    axios.post(this.host + '/auth' + '/addresses/', this.form_address, {
                        headers: {
                            'Authorization': 'Bearer ' + this.token
                        },
                        responseType: 'json'
                    ...
                    // 修改地址
                    axios.put(this.host + '/auth' + '/addresses/' + this.addresses[this.editing_address_index].id + '/', this.form_address, {
                        headers: {
                            'Authorization': 'Bearer ' + this.token
                        },
                        responseType: 'json'
        ...
        del_address: function (index) {
            axios.delete(this.host + '/auth' + '/addresses/' + this.addresses[index].id + '/', {
                headers: {
                    'Authorization': 'Bearer ' + this.token
                },
                responseType: 'json'
        ...
        set_default: function (index) {
            axios.put(this.host + '/auth' + '/addresses/' + this.addresses[index].id + '/status/', {}, {
                headers: {
                    'Authorization': 'Bearer ' + this.token
                },
                responseType: 'json'
        ...
        save_title: function (index) {
            if (!this.input_title) {
                ...
                axios.put(this.host + '/auth' + '/addresses/' + this.addresses[index].id + '/title/', {
